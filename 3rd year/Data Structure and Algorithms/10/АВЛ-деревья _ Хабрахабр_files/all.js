...
function userKarmaVote(link, target_id, target_type, vote){
  $.post('/json/vote/',{ti:target_id, tt:target_type, v:vote},function(json){
    if(json.messages == 'ok'){
      ...
      // выделим отмеченный пункт.
      if(vote === 1){
        $('.js-vote_plus',karma).addClass('voted').attr('title','Вы проголосовали положительно.');
        $('.js-vote_minus',karma).attr('title','Понизить карму.');
        $('.js-vote_minus',karma).removeClass('voted');
      }else if(vote === -1){
        $('.js-vote_minus',karma).addClass('voted').attr('title','Вы проголосовали отрицательно.');
        $('.js-vote_plus',karma).attr('title','Повысить карму.');
        $('.js-vote_plus',karma).removeClass('voted');
      }

      // обновим значение кармы
      $('.js-karma_num', karma).text(json.score);

      // обновим кол-во голосов:deprecated!!!
      $('.votes', karma).text(json.votes_count);
    }else{
      show_system_error(json);
    }
